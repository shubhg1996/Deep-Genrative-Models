...
def sample_gaussian(m, v):
    ...
    z = m + torch.sqrt(v) * torch.rand_like(m)
...
def log_normal(x, m, v):
    ...
    log_prob = torch.sum(torch.distributions.normal.Normal(m, torch.sqrt(v)).log_prob(x), dim=-1)
...
def log_normal_mixture(z, m, v):
    ...
    log_prob = log_mean_exp(log_normal(z.unsqueeze(1), m, v), 1)
...
def plot_grid(model):
    x_batch = model.sample_x(200).detach().cpu().numpy()
    x_batch = x_batch.reshape(10, 20, 28, 28).swapaxes(1,2).reshape(10*28,20*28)
    plt.imsave("temp_vis.png", x_batch, cmap='gray')
    
def plot_grid_fsvae(model):
    z = model.sample_z(20)
    z = duplicate(z, 10)
    y = z.new(np.repeat(np.eye(10), 20, 0))
    x_batch = model.compute_mean_given(z, y)
    save_image(x_batch.view(200, 3, 32, 32), 'temp_vis.png', nrow=20)
