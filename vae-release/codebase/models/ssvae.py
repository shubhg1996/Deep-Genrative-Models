...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        rec = torch.mean(torch.sum(y_prob*rec_ls.reshape(N_batches, -1), dim=1))

        kl_y = torch.mean(ut.kl_cat(y_prob, y_logprob, torch.log(torch.ones_like(y_prob)/self.y_dim)))

        kl_z_ls = ut.kl_normal(q_mu, q_var, torch.zeros_like(q_mu), torch.ones_like(q_var))

        kl_z = torch.mean(torch.sum(y_prob*kl_z_ls.reshape(N_batches, -1), dim=1))
