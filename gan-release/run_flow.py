...
if __name__ == '__main__':
	...
	parser.add_argument('--n_flows', default=5, type=int, 
		help='number of planar flow layers')
	...
	parser.add_argument('--batch_size', default=100, type=int, 
		help='batch size')
	parser.add_argument('--out_dir', default='maf', 
	...
	# load model
	model = MAF(input_size=2, hidden_size=args.hidden_size, n_hidden=1, n_flows=args.n_flows).to(device)
	...
	for epoch in range(1, args.n_epochs+1):
		...
		if is_best:
			best_epoch = epoch
		print('Best validation log-likelihood at epoch {}: {:.6f}'.format(best_epoch, -best_loss))
