...
class MADE(nn.Module):
	...
	def forward(self, z):
		...
		for i in range(self.input_size):
			mu, a = self.net(x.clone()).chunk(2,1)
			x[:, i] = mu[:, i] + z[:, i]*torch.exp(a[:, i])
		log_det = -a.sum(-1, keepdim=True)
		# YOUR CODE ENDS HERE
...
class MAF(nn.Module):
	...
	def log_probs(self, x):
		...
		"""
		# YOUR CODE STARTS HERE
		log_dets = torch.zeros(x.size(0), 1, device=x.device)
		for made in self.nf:
			x, log_det = made.inverse(x)
			log_dets += log_det
		z = x
		# print("TEST:", self.base_dist.log_prob(z))
		# exit(0)
		log_prob = torch.sum(self.base_dist.log_prob(z), dim=-1, keepdim=True) + log_dets
